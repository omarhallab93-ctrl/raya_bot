...
async def roulette_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("حدد مبلغ الرهان! مثال: `/roulette_bet 100`", parse_mode="Markdown")
        return
...
async def set_guess_private(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        ...
        return
    if not context.args:
        await update.message.reply_text("حدد الكلمة! مثال: `/guess تفاحة 200`", parse_mode="Markdown")
        return
    ...
    await update.message.reply_text(f"تم حفظ الكلمة: **{secret_word}**! اذهب للجروب واكتب `/start_guess`", parse_mode="Markdown")
...
async def activate_guess_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pending:
        await update.message.reply_text("اذهب للخاص واكتب `/guess الكلمة` أولاً!")
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "▫️ `/unmute` - فك الكتم\n\n"
        "🎰 **الألعاب والقرعة:**\n"
        "▫️ `/roulette` - لعبة الروليت الروسي (كتم عشوائي)\n"
        "▫️ `/roulette_bet [المبلغ]` - روليت المراهنات بالفلوس\n"
        "▫️ `/guess [الكلمة]` - وضع كلمة سرية (في الخاص)\n"
        "▫️ `/start_guess` - بدء التخمين في الشات\n"
...
def main():
    ...
    app.add_handler(CommandHandler("roulette", russian_roulette))
    app.add_handler(CommandHandler("roulette_bet", roulette_bet))
    app.add_handler(CommandHandler("guess", set_guess_private))
    app.add_handler(CommandHandler("start_guess", activate_guess_group))
